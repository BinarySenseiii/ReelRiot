...
export const getTmdbMovieDetail = (imdbCode: string) =>
  `/find/${imdbCode}?api_key=${API_KEY}&language=en-US&external_source=imdb_id`;

export const getMovieImages = (movieId: string) =>
  `/movie/${movieId}/images?api_key=${API_KEY}&append_to_response=images`;

export const getMovieReviews = (movieId: string) => `/movie/${movieId}/reviews?api_key=${API_KEY}`;
export const getMovieVideos = (movieId: string) => `/movie/${movieId}/videos?api_key=${API_KEY}`;
