...
const MovieTorrents: React.FC<MovieTorrentsProps> = ({ isTitle, torrents, imdb_code }) => (
	<Stack spacing="xs">
				...
				<CustomTooltip label={torrent.size} key={torrent.date_uploaded_unix}>
					<Anchor href={torrent.url}>
						<Button size="sm" compact variant="white" color="dark" fw={500}>
							{torrent.quality}.<span className="uppercase">{torrent.type}</span>{' '}
							{Number(torrent.bit_depth) > 9 && (
								<span className="text-xs ml-1 font-bold" style={{ color: 'red' }}>
									{torrent.video_codec}
								</span>
							)}
						</Button>
		...
		<Anchor href={`https://yifysubtitles.org/movie-imdb/${imdb_code}`} target="_blank">
